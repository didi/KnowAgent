...
​	基于上述原因，决定自研采集引擎 - Know Agent与对应管理平台 Agent Manager。

# 整体设计

​		Know Agent 是基于 Java 的高性能、支持多租户隔离的采集引擎，它能在任意情况下保证采集数据的完整性，持续采集速度可达150MB+/秒。对于 Source、Sink 与 Log Event 处理器具备良好的可扩展性。

​		Agent Manager 是针对 Know Agent 的管理平台。用于管理相关元数据、Agent与采集任务，Know Agent 与采集任务的健康度自动巡检、故障诊断，指标展示。

![logi-am架构](assets/know agent总体设计.png)	

​	Logi-AM 最小外部依赖组件有 2 个：

1. MySQL：用于存储 Agent Manager 元数据，与 Know Agent 上报的指标、错误日志数据（Agent上报的指标、错误日志支持Elasticsearch存储以应对大规模Know Agent集群管控）。
2. Kafka：作为采集的日志数据、以及Agent上报的指标、错误日志数据的消息总线。

​	夜莺平台：非必须依赖，用于执行Agent Manager下发的Agent安装、升级、卸载指令，Agent Manager支持通过API方式扩展其他第三方自动化部署平台。Agent Manager 支持 Know Agent 自动注册方式进行接入。
...
​	Agent Manager 启动后会持续消费 Know Agent 指标流、错误日志流对应的 Topic，然后将指标、错误日志数据根据配置写入对应存储（默认存储为MySQL，存储类型支持：MySQL、Elasticsearch，并支持通过接口方式扩展其他存储），Agent Manager 将根据这些数据对所管控的Agent、采集任务进行健康度巡检、故障诊断，以及指标展示。

## Know Agent 架构

![know agent架构](assets/know agent架构.png)
...
​	高可靠、高性能、易扩展、具备全方位可观测性的采集引擎，与面向服务的易管、易观、易治理的管理平台，大幅降低日志数据采集接入成本，大幅提升日志数据采集接入效率。

### 易融入流式计算

​	面向服务的采集任务、高可靠的采集引擎可确保在任意情况下的数据完整性、完善的监控指标，支撑可靠的采集任务的数据完整性校验，这使得采集任务可作为流式计算的一种可靠的数据源，形成整个流式计算链路的数据完整性闭环。
...
​	良好的代码抽象设计，使得采集引擎的Source/Sink/Interceptor易于开发。管理平台亦具备良好的领域设计，使得扩展与二开非常容易。
...
- 支持目录型 File Source 采集
- Source：syslog、http、JDBC
- Sink：pulsar、clickhouse、influxdb、hive
- 支持更多的序列化协议
- 支持更多平台的数据采集

**Agent-Manager**

- 支持目录型 File Source 采集任务的健康度巡检、故障诊断
- 支持更多类型的接收端配置
- 支持更多类型的采集任务配置
- 更好的采集任务配置体验
- 更优秀的可视化体验
...
​	Know Agent 与 Agent-Manager 已经开源（项目地址：https://github.com/didi/LogiAM），作为在滴滴内部经过多年的大量复杂、核心场景验证过的采集引擎与管控平台，我们会持续对其进行核心业务抽象，回馈社区，我们也希望热心的社区同学和我们交流想法，共同提升 Know Agent 与 Agent-Manager 的功能和体验。
