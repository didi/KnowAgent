...
public enum LogCollectTaskHealthInspectionResultEnum {
    ...
    LOG_PATH_COLLECT_DELAYED_CAUSE_BY_DATA_SEND_FAILED(19, "下游接收端写入失败导致的日志采集任务延时", LogCollectTaskHealthLevelEnum.RED, false),
    LOG_PATH_COLLECT_DELAYED_CAUSE_BY_HOST_CPU_USAGE_LIMIT_EXISTS_CAUSE_BY_AGENT_FULL_GC_OVER_FREQUENCY(20, "agent触发cpu阈值限流导致的日志采集任务延时，限流原因为：agent full gc 过频", LogCollectTaskHealthLevelEnum.RED, false),
    LOG_PATH_COLLECT_DELAYED_CAUSE_BY_HOST_CPU_USAGE_LIMIT_EXISTS(21, "agent触发cpu阈值限流导致的日志采集任务延时", LogCollectTaskHealthLevelEnum.RED, false),
    LOG_PATH_COLLECT_DELAYED_CAUSE_BY_SINK_SLOW(22, "agent sink端发送速率瓶颈导致的日志采集任务延时", LogCollectTaskHealthLevelEnum.RED, false),
    LOG_PATH_COLLECT_DELAYED_CAUSE_BY_SOURCE_SLOW(23, "agent source端采集速率瓶颈导致的日志采集任务延时", LogCollectTaskHealthLevelEnum.RED, false),
    ...
    HOST_CPU_USAGE_LIMIT_EXISTS(12, "日志采集任务在对应主机端存在agent cpu阀值限流", LogCollectTaskHealthLevelEnum.YELLOW, true),
    HOST_CPU_USAGE_LIMIT_EXISTS_CAUSE_BY_AGENT_FULL_GC_OVER_FREQUENCY(19, "日志采集任务在对应主机端存在因agent full gc过频导致cpu使用率突破限流阈值引发的agent cpu阀值限流", LogCollectTaskHealthLevelEnum.YELLOW, true),
    ...
    HOST_UNABLE_CONNECT(16, "待采集主机故障-无法连通", LogCollectTaskHealthLevelEnum.RED, true),
