...
import { Select, Form, Input, Switch, Radio, DatePicker, Row, Col, Collapse } from 'antd';
...
interface ICollectObjectProps extends FormComponentProps {
  collectMode: number;
  openHistory: boolean;
...
const CollectObjectConfiguration = (props: Props & ICollectObjectProps) => {
  ...
  const [serviceList, setServiceList] = useState([] as IService[]);
  ...
  const getSuHostlist = (appId: number) => {
    getHostListbyServiceId(appId).then((res: any) => {
  ...
  return (
    <div className="set-up collection-object">
      <Form {...contentFormItemLayout}>
        <Form.Item label="日志采集任务名">
          {getFieldDecorator('step1_logCollectTaskName', {
            initialValue: '',
            rules: [{
              required: true,
...
export default connect(null, mapDispatchToProps)(CollectObjectConfiguration);
