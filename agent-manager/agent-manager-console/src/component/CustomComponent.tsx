    ...
    <>
      <span className="table-operation">
        {freeBtns.map((item, index) => {
          ...
          if (item.needTooltip) {
            return (
              <Tooltip placement="topLeft" title={item.tooltipText ? item.tooltipText : '更多功能请关注商业版'}>
                <span key={index}><a style={{ color: '#00000040' }}>{item.label}</a></span>
              </Tooltip>
            )
          }
          ...
          if (item.clickFunc) {
            return (<a type="javascript;" key={index} onClick={() => item.clickFunc(record)}>{item.label}</a>);
          }
          ...
          return (<span key={index}><a>{item.label}</a></span>);
        })}
    ...
    <Link to={
      ...
      {props.needToolTip ? <Tooltip key={props.key} placement="bottomLeft" title={props.element}>
        {typeof (props.element) === 'string' ?
          props?.element?.length > 26 ? props?.element.substring(0, 26) + '...' : props?.element
          : props.element}
