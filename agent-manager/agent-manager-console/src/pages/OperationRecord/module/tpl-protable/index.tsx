...
import customHandle from './module';
...
const ProTableMoudle = (props: any) => {
  ...
  const handleContentFormModal = (btnConfig, optType?: string) => {
    setPageTreeData({ ...btnConfig, ...props, genData, selectRowKeys, optType });
  ...
  const handleCustomContentFormModal = (btnConfig) => {
    setPageTreeData({ ...btnConfig, ...props, genData, selectRowKeys });
    if (selectRowKeys.length < 1 && btnConfig.isBatch) {
      message.warning('请勾选要操作的数据');
      return;
    }
    ...
    import(`${path}`).then((res) => {
      setCustomContent({ CustomContent: res[btnConfig?.customPageName] });
    });
  };
  ...
  const handleConfirmContentFormModal = (btnConfig) => {
    ...
    if (btnConfig.customRenderConfirm === 1 || btnConfig.containerType === 'custom') {
      const path = './module';
      import(`${path}`).then((res) => {
        res[btnConfig?.customPageName]({ ...btnConfig, ...props, genData, selectRowKeys });
      });
    } else {
  ...
  const setQueryFormArray = async (queryFormArray) => {
    ...
    for (let i = 0; i <= queryFormArrayData.length - 1; i++) {
      const item = queryFormArrayData[i];
      if (item?.type === 'select') {
        const handleList = await import(`${path}`);
  ...
  const querySearch = (values: any) => {
    genData({ ...values, queryTerm: searchResult });
    setAllParams(values);
  };
